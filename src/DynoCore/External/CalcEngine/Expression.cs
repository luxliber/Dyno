...
namespace CalcEngine
{
                ...
                case TKID.ADD:
                    {
                        ...
                        if (_lft._token.Type == TKTYPE.IDENTIFIER && double.TryParse(_lft, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out double x))
                            lft = (double)_lft;
                        else if (_lft._token.Value is double)
                            lft = (double)_lft;
                        else
                            lft = (string)_lft;

                        if (_rgt._token.Type == TKTYPE.IDENTIFIER && double.TryParse(_rgt, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out x))
                            rgt = (double)_rgt;
                        else if (_rgt._token.Value is double)
                            rgt = (double)_rgt;
                        else
                            rgt = (string)_rgt;

                        if (lft is double && rgt is double)
                            return (double) lft + (double) rgt;

                        return lft.ToString() + rgt;
                    }
                case TKID.SUB:
    ...
    class VariableExpression : Expression
    {
        ...
        public VariableExpression(IDictionary<string, object> dct, string name, object value) : base(value)
        {
            _dct = dct;
            _name = name;
        }
