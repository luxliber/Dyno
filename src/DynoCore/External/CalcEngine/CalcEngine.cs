...
namespace CalcEngine
{
	...
	public partial class CalcEngine
	{
        ...
        public void RegisterFunction(string functionName, int parmMin, int parmMax, CalcEngineFunction fn, string cat="Core")
        {
            _fnTbl.Add(functionName, new FunctionDefinition(parmMin, parmMax, fn, cat));
        }
        ...
        /// <param name="fn">Delegate that evaluates the function.</param>
        /// <param name="cat"></param>
        public void RegisterFunction(string functionName, int parmCount, CalcEngineFunction fn, string cat="Core")
        {
            RegisterFunction(functionName, parmCount, parmCount, fn, cat);
        }
        /// <summary>
		...
		Expression ParseAtom()
		{
			...
			switch (_token.Type)
			{
                ...
                case TKTYPE.IDENTIFIER:
                    ...
                    {
                        x = new VariableExpression(_vars, id, _vars[id]);
                        break;
                    }
        ...
        void GetToken()
        {
            ...
            var isLetter = char.IsLetter(c);//(c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
            var isDigit = c >= '0' && c <= '9';
			if (!isLetter && !isDigit)
			{
            ...
            {
                c = _expr[_ptr + i];
                isLetter = char.IsLetter(c);//c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
                isDigit = c >= '0' && c <= '9';
